...
const ConceptosSalarialesModal = ({ onClose }) => {
    ...
    const loadConceptos = async () => {
        ...
        try {
            setLoading(true);
            const response = await getConceptosSalariales({ activo: 'true', espacioTrabajoId, limit: 1000 });
                    ...
                    <div className="modal-body" style={{ padding: '2rem' }}>
                        ...
                        {espacioTrabajoId && (
                            <>
                                ...
                                {/* Listados responsivos (se apilan en pantallas pequeñas) */}
                                <div style={{
                                    display: 'grid',
                                    ...
                                    <div>
                                        ...
                                        {loading ? (
                                            ...
                                            <div className="table-container">
                                                <table className="table" style={{ border: '1px solid var(--border-color)', minWidth: 'auto' }}>
                                                    <thead>
                                                        <tr>
                                                            <th>Nombre</th>
                                                            <th>Valor</th>
                                                    ...
                                                    <tbody>
                                                        {remunerativos.map(c => (
                                                            <tr key={c.id}>
                                                                <td style={{ whiteSpace: 'normal', wordBreak: 'break-word', minWidth: '120px' }}>
                                                                    <strong>{c.nombre}</strong>
                                                                </td>
                                                                <td style={{ fontWeight: 600 }}>{c.esPorcentaje ? `${c.valor}%` : `$${c.valor}`}</td>
                                                                <td>
                                                                    <div className="table-actions" style={{ justifyContent: 'center' }}>
                                                                        <button className="btn btn-warning btn-sm" onClick={() => handleEdit(c)} title="Editar">
                                                                        ...
                                                                        {!c.esObligatorio && (
                                                                            <button className="btn btn-danger btn-sm" onClick={() => handleDeleteClick(c)} title="Eliminar">
                                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" style={{ width: 14, height: 14 }}>
