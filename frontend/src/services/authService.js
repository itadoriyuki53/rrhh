/**
 * @fileoverview Servicio de autenticaciÃ³n y gestiÃ³n de sesiÃ³n.
...
export const login = async (credentials) => {
    ...
    if (!response.ok) throw new Error(result.error || 'Error al iniciar sesiÃ³n');
    return result;
};

/**
 * Cierra la sesiÃ³n del usuario actual en el servidor.
 *
 * @returns {Promise<Object>} Respuesta de confirmaciÃ³n del servidor.
 * @throws {Error} Si ocurre un error al cerrar sesiÃ³n.
 */
export const logout = async () => {
    const response = await httpClient(`${API_URL}/auth/logout`, { method: 'POST' });
    const result = await response.json();
    if (!response.ok) throw new Error(result.error || 'Error al cerrar sesiÃ³n');
    return result;
};
 ...
 * @throws {Error} Si los datos son invÃ¡lidos o el email ya existe.
 */
 ...
 *
 * @returns {Promise<Object|null>} Datos del usuario o `null` si no hay sesiÃ³n activa.
 ...
 *
 * @param {{ contrasenaActual: string, nuevaContrasena: string }} passwordData - Datos de cambio de contraseÃ±a.
 * @returns {Promise<Object>} Respuesta de confirmaciÃ³n.
 * @throws {Error} Si la contraseÃ±a actual es incorrecta.
 */
export const updatePassword = async (passwordData) => {
    ...
    const result = await response.json();
    if (!response.ok) throw new Error(result.error || 'Error al actualizar contraseÃ±a');
    return result;
};
